...
if( szConf.environment === 'node' )  exports.whichType = whichType;
/*+== whichType ==================
====================+*/
...
/**
 * @typedef {object} commandArgObj - コマンドライン引数の分析結果
 * @prop {Object.<string, number>} opt - スイッチを持つ引数について、スイッチ：値形式にしたオブジェクト
 * @prop {string[]} val - スイッチを持たない引数の配列
 */
/** analyzeArg: コマンドライン引数を分析
 * @param {void} - なし
 * @returns {commandArgObj} 分析結果のオブジェクト
 */
const analyzeArg = () => {
  const v = {whois:szConf.whois+'.analyzeArg'};
  let rv = {opt:{},val:[]};
  try {
    szLog([v.whois,szConf.log.start],v); // 開始ログを出力
...
if( szConf.environment === 'node' )  exports.analyzeArg = analyzeArg;
...
const analyzePath = (arg) => {
  const v = {whois:szConf.whois+'.analyzePath'};
  let rv = {full:arg};  // 戻り値。Errorの場合は書き換えなのでletで宣言
  try {
...
if( szConf.environment === 'node' )  exports.analyzePath = analyzePath;
