...
function refreshMaster() {
  const v = { whois: 'refreshMaster', rv: null, cols:[
    ...
    'type','prefix','label','suffix',
    // 手動追加項目(手動で追記が必要な項目)
    'date','abstract','price','payby','note'
  ],colnum:{}};
  ...
  try {
    ...
    v.parts = ['=arrayformula(_)',
      ...
      'if(isblank(p2:p),true,_)',	// dateが空欄なら対象
      'if(isblank(q2:q),true,_)',	// abstractが空欄なら対象
      'if(isblank(r2:r),true,_)',	// priceが空欄なら対象
      'if(isblank(s2:s),true,false)',	// paybyが空欄なら対象
    ];
...
function determineType(filename) {
  const v = { whois: 'determineType', rv: null, list:[{
    ...
    f:x=>{return {type:'YFP税務顧問報酬',prefix:`20${x[1]}/${('0'+x[2]).slice(-2)}`,label:'顧問報酬'};}
  },{
    rex:/^(20\d{2})(\d{2})(\d{2})_400_003\.pdf$/,
    f:x=>{return {type:'YFP記帳代行報酬',prefix:`20${x[1]}/${('0'+x[2]).slice(-2)}`,label:'記帳代行'};}
  },{
    rex:/^(20\d{2})(\d{2})\.pdf$/,
    f:x=>{return {type:'AMEX',label:`20${x[1]}/${('0'+x[2]).slice(-2)}`};},
  },{
    rex:/^EF(20\d{2})(\d{2})\.pdf$/,
    f:x=>{return {type:'恵比寿',label:`20${x[1]}/${('0'+x[2]).slice(-2)}`}}
  },{
    rex:/^CK(20\d{2})(\d{2})\.pdf$/,
    f:x=>{return {type:'上池袋',label:`20${x[1]}/${('0'+x[2]).slice(-2)}`}}
  },{
    rex:/^HS(20\d{2})(\d{2})\.pdf$/,
    f:x=>{return {type:'羽沢',label:`20${x[1]}/${('0'+x[2]).slice(-2)}`}}
  },{
    rex:/^MUFG(\d{2})(\d{2})\.pdf$/,
    f:x=>{return {type:'MUFG通帳',prefix:`MUFG vol.${('0'+x[1]).slice(-2)} `,label:'p.'+('0'+x[2]).slice(-2)}}
  },{
    rex:/^SMBC(\d{2})(\d{2})\.pdf$/,
    f:x=>{return {type:'SMBC通帳',prefix:`SMBC vol.${('0'+x[1]).slice(-2)} `,label:'p.'+('0'+x[2]).slice(-2)}}
  },{
    rex:/^note(20\d{2})(\d{2})\.pdf$/,
    f:x=>{return {type:'確証貼付ノート',label:`p.${('0'+x[2]).slice(-2)}`}}
  },{
    rex:/^pension(20\d{2})(\d{2})\.pdf$$/,
    f:x=>{return {type:'健保・年金',label:`20${x[1]}/${('0'+x[2]).slice(-2)}`}}
  }]};
  dev.start(v.whois, [...arguments]);
  try {
