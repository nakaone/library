      ...
      const sdbTable = class {
        ...
        constructor(arg){
          ...
          try {
            ...
            // ------------------------------------------------
            /*
            v.step = 6.1; // メンバ初期化後、schema.colsからunique/auto_increment項目を追加
            this.schema.unique = {};
            this.schema.auto_increment = {};
            for( v.i=0 ; v.i<this.schema.cols.length ; v.i++ ){
              console.log(`l.483 this.schema.cols[${v.i}]=${JSON.stringify(this.schema.cols[v.i])}`
              + `\nprimaryKey(${whichType(this.schema.cols[v.i].primaryKey)})=${this.schema.cols[v.i].primaryKey}`
              + `\nunique(${whichType(this.schema.cols[v.i].unique)})=${this.schema.cols[v.i].unique}`);

              v.step = 6.11; // primaryKey属性、またはunique属性項目の場合、this.schema.uniqueに登録
              ...
              if( v.bool(this.schema.cols[v.i].primaryKey) || v.bool(this.schema.cols[v.i].unique) )
                this.schema.unique[this.schema.cols[v.i].name] = [];

              v.step = 6.12; // auto_increment属性項目の場合、this.schema.auto_incrementに登録
              v.ai = String(this.schema.cols[v.i].auto_increment).toLowerCase() || 'null';
              v.o = null;
              if( v.ai.match(/true/) ){
                v.step = 6.13;  // 真偽値trueで指定された場合
                v.o = {base:1,step:1,current:1};
              } else if( v.ai.match(/^\d+$/) ){
                v.step = 6.14;  // 数値で指定された場合
                v.o = {base:Number(v.ai),step:1,current:this.base};
              } else {
                v.m = v.ai.match(/\[(\d+?),?(\d*)\]/);
                console.log(`l.506`,v.m)
                if( v.m ){
                  v.step = 6.15;  // 配列で指定された場合
                  v.o = {base:Number(v.m[1]),step:Number(v.m[2])||1,current:this.base};
                }
              }
              if( v.o !== null ){
                v.step = 6.16; // 有効な指定が有った場合は登録
                this.schema.auto_increment[this.schema.cols[v.i].name] = v.o;
                vlog(this.schema,'auto_increment',v)
              }
            }
            v.step = 6.19;

            v.step = 6.2; // データスキャンの結果を反映
            v.unique = Object.keys(this.schema.unique);
            v.auto_increment = Object.keys(this.schema.auto_increment);
            vlog(this.schema,['unique','auto_increment'],v);
            vlog(v,['unique','auto_increment']);
            for( v.i=0 ; v.i<this.values.length ; v.i++ ){

              v.step = 6.21; // this.schema.uniqueに設定されている値をMapに追加
              for( v.j=0 ; v.j<v.unique.length ; v.j++ ){
                v.map = this.schema.unique[v.unique[v.j]];
                v.val = this.values[v.i][v.unique[v.j]];
                vlog(v,['map','val'],534);  // 全てのmapは空配列、valはundefined
                if( v.val === undefined ) continue;
                if( v.map.indexOf(v.val) < 0 ){
                  v.map.push(v.val);
                } else {
                  throw new Error(`${v.whois}:「${v.unique[v.j]}」欄の値"${v.val}"は重複しています`);
                }
              }

              v.step = 6.22; // this.schema.auto_incrementの最大(小)値をcurrentにセット
            ...
            */
            ...
            /* sdbTable.constructor abnormal end at step.6.1
            this.schema.unique[unique].map.indexOf is not a function */
            //console.log(`l.556 this.values(${whichType(this.values)})=${stringify(this.values)}`);
            this.values.forEach(vObj => {
              //console.log(`l.558 vObj(${whichType(vObj)})=${stringify(vObj)}`);
              Object.keys(this.schema.unique).forEach(unique => {
                console.log(`l.560 vObj[${unique}](${whichType(vObj[unique])})=${stringify(vObj[unique])}`)
                if( vObj[unique] ){
                  console.log(`l.562 this.schema.unique[${unique}](${whichType(this.schema.unique[unique])})=${stringify(this.schema.unique[unique])}`)
                  console.log(this.schema.unique[unique].indexOf(vObj[unique]))
                  ...
                  console.log(`l.569 this.schema.unique[${unique}]=${stringify(this.schema.unique[unique])}`)
                }
              });
              ...
              Object.keys(this.schema.auto_increment).forEach(ai => {
                ...
                console.log(`l.581 this.schema.auto_increment[${ai}]=${stringify(this.schema.auto_increment[ai])}`)
              });
            });

            console.log(`l.585 unique=${JSON.stringify(this.schema.unique)}\nauto_increment=${JSON.stringify(this.schema.auto_increment)}`);
      ...
      const sdbSchema = class {
        ...
        constructor(arg={}){
          ...
          try {
            ...
            vlog(this,['primaryKey','unique','auto_increment','default'],758);
