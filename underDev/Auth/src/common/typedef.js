...
const typedef = {
  authAuditLog: {note:'authServerの監査ログ',
    type: 'Object',
    prop: [
      {name:'timestamp',type:'string',note:'要求日時。ISO8601拡張形式の文字列',default:'Date.now()'},
      ...
      {name:'result',type:'string',note:'サーバ側処理結果。fatal/warning/normal',default:'normal'},
  ...
  authErrorLog: {note:'authServerのエラーログ',
    ...
    prop: [
      ...
      {name:'memberId',type:'string',note:'メンバの識別子(=メールアドレス)'},
      {name:'deviceId',type:'string',note:'デバイスの識別子'},
      {name:'result',type:'string',note:'サーバ側処理結果。fatal/warning/normal',default:'fatal'},
  ...
  authIndexedDB: {note:['authClientKeysを継承した、クライアントのIndexedDBに保存するオブジェクト',
    ...
    prop: [
      ...
      {name:'deviceId',type:'string',note:'デバイスの識別子',default:'UUID'},
      ...
      {name:'SPkey',type:'string',note:'サーバ公開鍵(Base64)',default:null},
  ...
  authRequest: {note:'authClientからauthServerに送られる、暗号化前の処理要求オブジェクト',
    type: 'Object',
    prop: [
      {name:'memberId',type:'string',note:'メンバの識別子(=メールアドレス)'},
      {name:'deviceId',type:'string',note:'デバイスの識別子'},
  ...
  authResponse: {note:'authServerからauthClientに返される、暗号化前の処理結果オブジェクト',
    type: 'Object',
    prop: [
      {name:'timestamp',type:'number',note:'サーバ側処理日時。UNIX時刻',default:'Date.now()'},
      {name:'result',type:'string',note:'サーバ側処理結果。fatal/warning/normal',default:'normal'},
      {name:'message',type:'string',note:'サーバ側からの(エラー)メッセージ',isOpt:true},
      {name:'request',type:'authRequest',note:'処理要求オブジェクト',isOpt:true},
  ...
  decryptedRequest: {note:'encryptedRequestをcryptoServerで復号した処理要求オブジェクト',
    type: 'Object',
    prop: [
      {name:'result',type:'string',note:'処理結果。"fatal"(後続処理不要なエラー), "warning"(後続処理が必要なエラー), "normal"'},
  ...
  decryptedResponse: {note:'encryptedResponseをcryptoClientで復号した処理結果オブジェクト',
    type: 'Object',
    prop: [
  ...
  encryptedRequest: {note:['authClientからauthServerに送られる、暗号化された処理要求オブジェクト',
  ...
  encryptedResponse: {note:['authServerからauthClientに返される、暗号化された処理結果オブジェクト',
  ...
  Member: {note:'メンバ一覧(アカウント管理表)上のメンバ単位の管理情報',
    ...
    prop: [
      ...
      {name:'log',type:'string',note:'メンバの履歴情報(MemberLog)を保持するJSON文字列',default:'new MemberLog()'},
      {name:'profile',type:'string',note:'メンバの属性情報(MemberProfile)を保持するJSON文字列',default:'new MemberProfile()'},
  ...
  MemberDevice: {note:'メンバが使用する通信機器の情報(マルチデバイス対応)',
    ...
    prop: [
      ...
      {name:'CPkeyUpdated',type:'number',note:'最新のCPkeyが登録された日時',default:'Date.now()'},
      {name:'trial',type:'string',note:'ログイン試行関連情報オブジェクト(MemberTrial[])。シート保存時はJSON文字列',default:[]},
    ],
  },
