...
- authClient クラス仕様書
  - クラス概要が引用形式になっている(いまここ)
    - ∵「- 内発処理はローカル関数からの処理要求に先行して行う」の先頭スペースが無いためminIndent=0となり、余計なスペースが削除されていない
...
### 原因①：trimIndentの共通インデント削除で、6桁になるべきminIndentが0になっている
...
```
{
  "lines": [
    ...
    "      %% this.note %%",
    "",
    "      %% this.summary.length === 0 ? '' : `## <span id=\"authclient_summary\">🧭 authClient クラス 概要</span>\\n\\n- クロージャ関数ではなくクラスとして作成",
    "- 内発処理はローカル関数からの処理要求に先行して行う` %%"
  ],
  "filter": [
    "      %% this.desc %%",
...
⇒ 解決案
- 導出は無いのでconstructorでいいけどクラス概要を関数として独立、結果を埋め込むように変更
