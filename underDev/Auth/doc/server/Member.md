...
## <a href="#Member_top"><span id="Member_desc">🧾 Member 概説</span></a>
...
constructor: 該当メンバのMemberインスタンス作成<br>- 新規メンバ(SPkey要求時)は新規作成<br>@constructor<br>@param {authServerConfig} config - authServerの設定値<br>@param {authRequest} request - 処理要求<br><br>
...
### <a href="#Member-addTrial_top"><span id="Member-addTrial_desc">🧾 addTrial 概説</span></a>

addTrial: 新しい試行を登録し、メンバにパスコード通知メールを発信<br>@param {authRequest} request - 処理要求<br>@returns {authResponse|Error}<br><br>
...
checkPasscode: 認証時のパスコードチェック<br>入力されたパスコードをチェック、Member内部の各種メンバの値を更新<br>@param {authRequest} request - 処理要求<br>@returns {authResponse|Error}<br><br>
...
### <a href="#Member-getMember_top"><span id="Member-getMember_desc">🧾 getMember 概説</span></a>

getMember: 指定メンバの情報をmemberListシートから取得<br>@param {string} memberId - ユーザ識別子(メールアドレス)	<br>@returns {Member|Error}  いまここ：元は"authResponse"だったが、"Member"の方がベター？<br><br>
...
judgeMember: 加入審査画面から審査結果入力＋結果通知<br>加入審査画面を呼び出し、管理者が記入した結果をmemberListに登録、審査結果をメンバに通知<br>memberListシートのGoogle Spreadのメニューから管理者が実行することを想定<br>@param {string} arg - 引数<br>@returns {null|Error} 戻り値<br><br>
...
### <a href="#Member-judgeStatus_top"><span id="Member-judgeStatus_desc">🧾 judgeStatus 概説</span></a>

judgeStatus: 指定メンバ・デバイスの状態を状態決定表により判定<br>@param {string} arg - 引数<br>@returns {null|Error} 戻り値<br><br>
...
### <a href="#Member-MemberDevice_top"><span id="Member-MemberDevice_desc">🧾 MemberDevice 概説</span></a>

MemberDevice: デバイス情報の初期オブジェクト生成<br>@returns {MemberDevice}<br><br>
## <span id="Member-MemberLog_top">🧩 MemberLog()</span>

### <a href="#Member-MemberLog_top"><span id="Member-MemberLog_desc">🧾 MemberLog 概説</span></a>

MemberLog: メンバ履歴情報初期化<br>@returns {MemberLog}<br><br>
## <span id="Member-MemberProfile_top">🧩 MemberProfile()</span>

### <a href="#Member-MemberProfile_top"><span id="Member-MemberProfile_desc">🧾 MemberProfile 概説</span></a>

MemberProfile: メンバ属性情報初期化<br>@returns {MemberProfile}<br><br>
## <span id="Member-reissuePasscode_top">🧩 reissuePasscode()</span>

reissuePasscode: パスコードを再発行する

### <a href="#Member-reissuePasscode_top"><span id="Member-reissuePasscode_desc">🧾 reissuePasscode 概説</span></a>

reissuePasscode: パスコードを再発行する<br>@param {string} arg - 引数<br>@returns {null|Error} 戻り値<br><br>
...
removeMember: 登録中メンバをアカウント削除、または加入禁止にする<br>- memberListシートのGoogle Spreadのメニューから管理者が実行することを想定<br>@param {string} arg - 引数<br>@returns {null|Error} 戻り値<br><br>
...
restoreMember: 加入禁止(論理削除)されているメンバを復活させる<br>- memberListシートのGoogle Spreadのメニューから管理者が実行することを想定<br>@param {string} arg - 引数<br>@returns {null|Error} 戻り値<br><br>
...
setMember: 指定メンバ情報をmemberListシートに保存<br>- 登録済メンバの場合は更新、未登録の場合は新規登録(追加)を行う<br>@param {string} arg - 引数<br>@returns {null|Error} 戻り値<br><br>
...
unfreeze: 指定されたメンバ・デバイスの「凍結中」状態を強制的に解除<br>- 引数でmemberIdが指定されなかった場合、凍結中デバイス一覧の要求と看做す<br>- deviceIdの指定が無い場合、memberIdが使用する凍結中デバイス全てを対象とする<br>- memberListシートのGoogle Spreadのメニューから管理者が実行することを想定<br>@param {string} arg - 引数<br>@returns {null|Error} 戻り値<br><br>
...
### <a href="#Member-updateCPkey_top"><span id="Member-updateCPkey_desc">🧾 updateCPkey 概説</span></a>

updateCPkey: 対象メンバ・デバイスの公開鍵を更新<br>@param {string} arg - 引数<br>@returns {null|Error} 戻り値<br><br>
