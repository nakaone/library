  ...
  constructor(arg){
    ...
    try {
      ...
      v.arg = Object.assign({
        ...
        propKey: 'saveSpread', // {string} ScriptPropertyのキー名
      ...
      if( v.ScriptProperties !== null ){ // 2回目以降の実行時(タイマー起動)
        ...
        v.step = 3.2; // 作業用ファイルの内容をthis.dataに読み込み
        ...
        if( arg === false ){ // 強制停止
          ...
          v.step = 2.3; // this.dataにスプレッドシート関連情報をセット
          this.data = this.spreadProperties();

          v.step = 2.4; // シート名一覧(this.conf.sheetList)の作成、this.data.Sheetsに初期値設定
          ...
          v.step = 2.5; // 取得する属性一覧(this.conf.propList)の作成
