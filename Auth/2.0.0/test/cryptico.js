const Cryptico = require('./cryptico.min.js');
global.authServer = {

  initialize: function(passphrase='N54YL!A#!)m{3}e-',bits=2048){
    const v = {whois:'initialize',rv:null,step:0};
    console.log(`${v.whois} start.`);
    try {

      v.step = 1; // 鍵ペアの生成
      v.sKey = Cryptico.generateRSAKey(passphrase, bits);
      v.pKey = Cryptico.publicKeyString(v.sKey);

      v.step = 2; // プロパティサービスへの保存
      ['passphrase','sKey','pKey'].forEach(x => {
  ...
  confirmKeys: function(){
    const v = {whois:'confirmKeys',rv:null,step:0};
    console.log(`${v.whois} start.`);
    try {

      v.step = 1; // キーの確認
      ['passphrase','sKey','pKey'].forEach(x => {
        console.log(PropertiesService.getDocumentProperties().getProperty(x));
      });
