...
function doTest(sce = 'dev', start = 0, num = 1) { // sce='all'->全パターン、num=0->start以降全部、num<0->start無視して後ろから
  ...
  function setupEnvironment(arg, src = sample) {  // setupEnvironment: テストに必要なシートを準備
     ...
     */
    const v = {
      whois: 'doTest.setupEnvironment', rv: null,
  ...
  try {
    ...
    for (v.sno = 0; v.sno < v.scenarioList.length; v.sno++) {
      ...
      for (v.idx = v.st; v.idx < v.ed; v.idx++) {
        ...
        dev.step(3.1); // テスト環境の構築(シートの準備)
        // テスト開始時は変更履歴(log)シートを削除、それ以外の場合の変更履歴は追記にする
        ...
        if (false === dev.check({
          asis: v.ac.request(v.scenario.opt),
...
function authClient(query, option = {}) {
  ...
  function request(query) {
    const v = { whois: `${cv.whois}.request`, rv: null };
    dev.start(v.whois, [...arguments]);
    try {
