...
function SpreadDb(query=[],opt={}){
  ...
  /** updateRow: 領域に新規行を追加 : いまここ
   * @param {Object} any
   * @param {sdbTable} any.table - 操作対象のテーブル管理情報
   * @param {Object|Function|any} any.where - 対象レコードの判定条件。配列可
   * @param {Object|Function|any} any.record - 更新する値
   * @returns {sdbLog[]}
   *
   * - where句の指定方法
   *   - Object ⇒ {キー項目名:キー項目の値}形式で、key:valueに該当するレコードを更新
  ...
  function updateRow(trans=[]){
    const v = {whois:'sdbTable.updateRow',step:0,rv:[],whereStr:[],recordStr:[]};
