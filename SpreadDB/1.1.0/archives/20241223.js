...
function SpreadDb(query=[],opt={}){
  ...
  function convertRow(data,header=[]){
    const v = {whois:pv.whois+'.convertRow',step:0,rv:{raw:[],obj:[],header:[]}};
    ...
    try {
      ...
      if( Array.isArray(data[0]) ){
        ...
        v.rv.obj = data;
        // いまここ
        // 引数headerが指定されていなかった場合、ヘッダ項目を洗い出し
        // ヘッダ項目の並びで二次元配列を作成
        v.rv.header = header.length > 0 ? header : Object.keys(data[0]);
        for( v.map={},v.i=0 ; v.i<v.rv.header.length ; v.i++ ){
        ...
        for( v.i=0 ; v.i<data.length ; v.i++ ){
          ...
          for( v.j in data[v.i] ){
            ...
            console.log(`l.783 v.arr[${v.map[v.j]}] = data[${v.i}][${v.j}] = ${data[v.i][v.j]}`)
            v.arr[v.map[v.j]] = data[v.i][v.j];
          }
          console.log(`l.786 v.arr=${JSON.stringify(v.arr)}`);  // いまここ：v.arrが空配列
          v.rv.raw.push(v.arr);
        }
      }
